...
from app.models import Post, VisibilityEnum, Like, User
from app import db
...
def dashboard():
    # get posts
    posts = Post.query.filter((Post.visibility == VisibilityEnum.PUBLIC) | (
...
def create_post():
    if request.method == 'POST':
        ...
        post = Post(user_id=current_user.id, content=content, visibility=VisibilityEnum(visibility))
        db.session.add(post)
        db.session.commit()
