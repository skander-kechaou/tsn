...
class Authorization:
    ...
    An instance can be used with the test :class:`.Client` request methods' ``security``
    parameter to send the header in test requests.

    Depending on the security scheme, either :attr:`parameters` or :attr:`token` will be
    set. The ``Basic`` scheme's token is decoded into the ``username`` and ``password``
    parameters.

    For convenience, ``security["key"]`` and ``security.key`` both access the key in the
    :attr:`parameters` dict, along with ``security.get("key")`` and ``"key" in security``.

    .. versionchanged:: 2.3
        The ``token`` parameter and attribute was added to support security schemes that use
...
class WWWAuthenticate:
    ...
    Depending on the security scheme, either :attr:`parameters` or :attr:`token` should be
    set. The ``Basic`` scheme will encode ``username`` and ``password`` parameters to a
    token.

    For convenience, ``security["key"]`` and ``security.key`` both act on the :attr:`parameters`
    dict, and can be used to get, set, or delete parameters. ``security.get("key")`` and
    ``"key" in security`` are also provided.
